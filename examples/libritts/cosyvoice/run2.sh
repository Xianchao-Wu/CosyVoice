...
# TODO 这个是利用已有的speech token tokenizer model来处理各个文件
# 每个集合都有自己的 utt2speech_token.pt
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  for model in llm flow hifigan; do
    ...
    python -m ipdb cosyvoice/bin/average_model.py \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  python -m ipdb cosyvoice/bin/export_jit.py --model_dir $pretrained_model_dir
  python -m ipdb cosyvoice/bin/export_onnx.py --model_dir $pretrained_model_dir
fi
